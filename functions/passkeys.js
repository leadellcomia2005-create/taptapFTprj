...
const base64url = (buffer) => Buffer.from(buffer).toString("base64").replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/g, "");
const fromBase64url = (value = "") => {
  const normalized = String(value).replace(/-/g, "+").replace(/_/g, "/");
  return Buffer.from(normalized.padEnd(Math.ceil(normalized.length / 4) * 4, "="), "base64");
};
const cleanName = (value, fallback) => typeof value === "string" && value.trim() ? value.trim() : fallback;
const hashCode = (code, salt = randomBytes(16).toString("base64")) => ({ salt, hash: scryptSync(String(code), salt, 32).toString("base64") });
const backupCode = () => {
...
async function audit(db, user, action, details = {}) {
  await db.ref("auditLogs").push({ action: `passkey_${action}`, userId: user.uid, actorId: user.uid, method: "passkey", ...details, createdAt: Date.now() });
}
...
function verifyClientData(encoded, expectedType, expectedChallenge, context) {
  ...
  return { clientDataHash: createHash("sha256").update(clientDataJSON).digest() };
}
...
function verifyAuthenticatorData(authenticatorData, rpId, requireAttestedCredential = false) {
  ...
  if (actualRpIdHash.length !== rpIdHash.length || !timingSafeEqual(actualRpIdHash, rpIdHash)) throw new HttpError(400, "Passkey was not created for this site.");
...
function parseAttestedCredential(authenticatorData) {
  ...
  if (coseKey.get(1) !== 2 || coseKey.get(3) !== -7 || coseKey.get(-1) !== 1) throw new HttpError(400, "This passkey type is not supported yet.");
  const x = coseKey.get(-2);
  const y = coseKey.get(-3);
  if (!Buffer.isBuffer(x) || !Buffer.isBuffer(y) || x.length !== 32 || y.length !== 32) throw new HttpError(400, "Invalid passkey public key.");
  return { credentialId: base64url(credentialId), publicKeyJwk: { kty: "EC", crv: "P-256", x: base64url(x), y: base64url(y), ext: true }, coseAlg: -7 };
}

const passkeysFrom = (config = {}) => config.passkeys && typeof config.passkeys === "object" ? config.passkeys : {};
...
export async function beginPasskeyRegistration(db, user, req) {
  ...
  await db.ref(`twoFactor/${user.uid}/pendingPasskeyRegistration`).set({ challenge, rpId: context.rpId, origin: context.origin, expiresAt: Date.now() + challengeExpiryMs });
  return {
    publicKey: {
      challenge,
      rp: { name: "Taptap Foodtrip", id: context.rpId },
      user: { id: base64url(Buffer.from(user.uid)), name: user.email || profile.email || user.uid, displayName: profile.name || user.name || user.email || "Customer" },
      pubKeyCredParams: [{ type: "public-key", alg: -7 }],
      timeout: 60_000,
      attestation: "none",
      authenticatorSelection: { residentKey: "preferred", userVerification: "required" },
      excludeCredentials: Object.keys(passkeysFrom(config)).map((id) => ({ type: "public-key", id }))
    }
  };
}
...
export async function verifyPasskeyRegistration(db, user, input = {}, req) {
  ...
  await configRef.set({
    enabled: true,
    method: "passkey",
    passkeys: { [credential.credentialId]: { name: cleanName(input.name, `${profile.name || "Customer"} passkey`), credentialId: credential.credentialId, publicKeyJwk: credential.publicKeyJwk, coseAlg: credential.coseAlg, signCount, createdAt, lastUsedAt: createdAt } },
...
export async function beginPasskeyAuthentication(db, user, req) {
  ...
  if (config.locked) throw new HttpError(423, "This account is locked.");
  const allowCredentials = Object.keys(passkeysFrom(config)).map((id) => ({ type: "public-key", id }));
  if (allowCredentials.length === 0) throw new HttpError(409, "No passkey is registered for this account.");
  const challenge = base64url(randomBytes(32));
  await db.ref(`twoFactor/${user.uid}/pendingPasskeyAuthentication`).set({ challenge, rpId: context.rpId, origin: context.origin, expiresAt: Date.now() + challengeExpiryMs });
  return { publicKey: { challenge, rpId: context.rpId, allowCredentials, timeout: 60_000, userVerification: "required" } };
}
...
export async function verifyPasskeyAuthentication(db, user, input = {}, req) {
  ...
  if (previousCount > 0 && signCount > 0 && signCount <= previousCount) throw new HttpError(401, "Passkey sign-in was rejected. Try again.");
  const verifier = createVerify("SHA256");
  verifier.update(Buffer.concat([authenticatorData, clientDataHash]));
  verifier.end();
  if (!verifier.verify(createPublicKey({ key: stored.publicKeyJwk, format: "jwk" }), fromBase64url(input.response?.signature))) throw new HttpError(401, "Passkey sign-in failed.");
  const now = Date.now();
  await configRef.update({ failedAttempts: 0, locked: false, lockedAt: null, lastVerifiedAt: now, pendingPasskeyAuthentication: null, [`passkeys/${credentialId}/signCount`]: signCount, [`passkeys/${credentialId}/lastUsedAt`]: now });
