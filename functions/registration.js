...
async function writeRegistrationAudit(db, action, details = {}) {
  ...
  await db.ref(`auditLogs/${key}`).set({
    action,
...
export function validateCustomerRegistration(input = {}) {
  ...
  if (!Object.values(passwordChecklist(password)).every(Boolean)) throw new HttpError(400, "Use a stronger password.");
...
async function enforceRegistrationRateLimit(db, source) {
  ...
  for (const key of keys) {
    ...
    if (count >= maxAttemptsPerWindow) throw new HttpError(429, "Too many registration attempts. Please wait 15 minutes, then try again.");
    await ref.set({ windowStart: count ? windowStart : now, count: count + 1, lastAt: now, expiresAt: now + rateWindowMs });
  }
}
...
export async function createCustomerRegistration({ db, auth, input, req, sendVerificationEmail, appBaseUrl }) {
  ...
  try {
    ...
    return { uid: userRecord.uid, email: values.email, profilePath: `users/${userRecord.uid}`, verificationSent };
  } catch (error) {
    if (userRecord?.uid) {
      await Promise.all([
    ...
    if (error?.code === "auth/email-already-exists") throw new HttpError(409, "We could not create this account. Please check your details or try signing in.");
