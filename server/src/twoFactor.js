...
function encryptionKey() {
  ...
  if (key.length !== 32) {
    throw new HttpError(503, "Account security is not ready yet.");
  }
  return key;
}
...
function decrypt(payload) {
  if (!payload?.iv || !payload?.tag || !payload?.ciphertext) throw new HttpError(500, "Security app setup is unavailable.");
...
async function recordFailure(db, user, method, idToken) {
  ...
  throw new HttpError(locked ? 423 : 401, locked
    ? "Account locked after three failed security attempts. Ask an owner to unlock it."
...
export async function sendSmsCode(db, user, sendSms, purpose = "challenge") {
  ...
  if (purpose === "challenge" && (!config.enabled || config.method !== "sms")) {
    throw new HttpError(409, "SMS is not the selected security method.");
  }
...
export async function finishEnrollment(db, user, method, code, idToken) {
  ...
  if (method === "totp") {
    if (!config.pendingTotp || Number(config.pendingTotp.expiresAt) < Date.now()) {
      throw new HttpError(410, "Security app setup expired. Show a new setup code.");
    }
    ...
    throw new HttpError(400, "Unsupported security method.");
  }
...
export async function verifyChallenge(db, user, input = {}, idToken) {
  ...
  if (config.locked) throw new HttpError(423, "Account locked after three failed security attempts.");
